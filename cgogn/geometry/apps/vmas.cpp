...
#include <cgogn/geometry/ui_modules/vmas.h>
...
template <typename T> using SAttribute = typename cgogn::mesh_traits<Surface>::Attribute<T>;
using SVertex = typename cgogn::mesh_traits<Surface>::Vertex;

template <typename T> using PAttribute = typename cgogn::mesh_traits<Points>::Attribute<T>;
using PVertex = typename cgogn::mesh_traits<Points>::Vertex;

template <typename T> using NMAttribute = typename cgogn::mesh_traits<NonManifold>::Attribute<T>;
...
int main(int argc, char **argv)
{
    std::string filename;
    if (argc < 2)
        filename = std::string(DEFAULT_MESH_PATH) + std::string("chair.off");
    else
    ...
    cgogn::ui::VMAS<Surface, Points, NonManifold> sf(app);
    ...
    NonManifold *skeleton = mpnm.mesh(cgogn::remove_extension(mps.mesh_name(*s)) + "_skeleton");
