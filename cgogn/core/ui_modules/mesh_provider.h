...
template <typename MESH> class MeshProvider : public ProviderModule
{
    template <typename T> using Attribute = typename mesh_traits<MESH>::template Attribute<T>;
  ...
  public:
    ...
    template <typename FUNC> void foreach_mesh(const FUNC &f)
    {
  ...
  public:
    ...
    template <typename T> using attribute_changed_t = struct attribute_changed_t_ (*)(Attribute<T> *attribute);
    ...
    template <typename CELL> using cells_set_changed = struct cells_set_changed_ (*)(CellsSet<MESH, CELL> *set);

    template <typename T> void emit_attribute_changed(const MESH &m, Attribute<T> *attribute)
    {
        MeshData<MESH> &md = mesh_data(m);
    ...
    template <typename CELL> void emit_cells_set_changed(const MESH &m, CellsSet<MESH, CELL> *set)
    {
